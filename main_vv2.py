...
from losses.dice import DiceLoss, expand_as_one_hot, GeneralizedDiceLoss
...
class Solver(object):
    ...
    def train(self, epoch):
        ...
        for i, (imgs, gts) in enumerate(tqdm(self.train_loader)):
            ...
            ce_loss = nn.CrossEntropyLoss(reduction='mean', weight=weight)(outputs, gts.reshape(-1,128,128,128))
            # dice_loss = DiceLoss(sigmoid_normalization=False)(outputs, expand_as_one_hot(gts.reshape(-1,128,128,128),14))
            dice_loss = GeneralizedDiceLoss(sigmoid_normalization=False)(outputs, expand_as_one_hot(
                gts.reshape(-1, 128, 128, 128), 14))
...
def main(config):
    ...
    train_set = Probset3d(config.train_path,is_train=True, fold=5)
    valid_set = Probset3d(config.valid_path,is_train=False,fold=5 )
    test_set = Probset3d(config.test_path,is_train=False, fold=5)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_epochs', type=int, default=60)  # random hyperparam search
    parser.add_argument('--num_epochs_decay', type=int, default=25)  # random hyperparam search
    ...
    parser.add_argument('--load_model', type=str, default='/mnt/HDD/datasets/competitions/candidate/vnet/VNet-400-0.0001000-100-0.5000-vv-fold5-3-ce+dice.pkl')
    parser.add_argument('--comment', type=str, default='vv-fold5-d19-ce+dice-then-gdice+ce')
