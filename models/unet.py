...
class EncoderBlock(nn.Module):
    def __init__(self, in_f):
        ...
        self.downsample = nn.Conv2d(in_channels=in_f, out_channels=in_f,
...
class BottomBlock(nn.Module):
    def __init__(self, in_feat, out_feat):
        ...
        self.downsample = nn.Conv2d(in_channels=in_feat, out_channels=in_feat,
                                    kernel_size=2, stride=2, padding=0)
...
class DRUNet(nn.Module):
    def __init__(self, num_channels=1, num_classes = 23):
        ...
        self.down1 = nn.Sequential(
            ConvBlock(num_channels, num_feat[0]),
            ConvBlock(num_feat[0], num_feat[0])
        )
        self.down2 = EncoderBlock(num_feat[0]) # 64->128
        self.down3 = EncoderBlock(num_feat[1]) # 128->256
        self.down4 = EncoderBlock(num_feat[2]) # 256->512
        ...
        self.up4 = DencoderBlock(num_feat[1], num_feat[0])
