...
from metrics import *
from models.unet import XDRUNet #, R2U_Net, AttU_Net
#from models.u_net import U_Net

from losses.dice import DiceLoss, MulticlassJaccardLoss, expand_as_one_hot
from losses.focal import FocalLoss2d
import csv
...
class Solver(object):
    ...
    def build_model(self):
        if self.model_type == 'UNet':
            ###### to do ########
            self.net = XDRUNet(unet_num_c =1, small_unet_num_c=14, n_classes=23)
            #self.net.load_state_dict(torch.load('/mnt/HDD/datasets/competitions/unet/models_for_cls/UNet-100-0.0001000-50-0.5000-uu-fold4-reconnect.pkl'))
...
def main(config):
    ...
    train_set = UnetSet(config.train_path, fold=1 )
    valid_set = UnetSet(config.valid_path,is_train=False,fold=1)
    test_set = UnetSet(config.test_path,is_train=False, fold=1)

    train_loader = DataLoader(train_set, batch_size=config.batch_size,num_workers=config.num_workers,shuffle=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_epochs', type=int, default=200)  # random hyperparam search
    parser.add_argument('--num_epochs_decay', type=int, default=50)  # random hyperparam search
    ...
    parser.add_argument('--comment', type=str, default='uu-final-fold1')

    config = parser.parse_args()


    x = datetime.today()
    y = x.replace(day=x.day+1 , hour=5, minute=0, second=5, microsecond=0)
    delta_t = y - x
